...
Start by reviewing the assembly file. The functions `interrupts_global_disable` and `interrupts_global_enable`
...
The `main` function in `code/button/button.c` sits in a loop, waiting for a button press and then redrawing the screen. Fill in the implementation of the empty `wait_for_click` function to operate by _polling_. It should:
