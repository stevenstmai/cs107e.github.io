...
All that remains for assignment 7 is make it so you can type faster without
your system dropping keystrokes.  This is all bare-metal code you wrote 
...
Start by wr out the simple getter functions that wrap the underlying
...
pixel's data in the framebuffer based on __pitch__, not __width__, because the
GPU may have made each row a little wider than requested for reasons of
alignment.

When accessing the pixel data, be ever mindful that C provides no
...
various dire consequences to follow. It is imperative that your functions
detect and reject an attempt to read or write a pixel location that is
out-of-bounds.

Be sure to draw only those pixels which lie within the framebuffer
bounds. You can enforce clipping by calling `gl_draw_pixel` for each pixel and
letting that function sort out whether the pixel is in bounds. This simple
approach is easy to get correct, but will be a bit pokey because of the
repeated checks.
...
your to confirm the full range of functionality for the module. You can rig up
an assert-based test that makes a `draw_xxx` call followed by some calls to
`read_pixel` to confirm the pixel color at certain locations, but this can get
unwieldy at scale. You may find it easier to "eyeball" drawing tests -- draw
...
As the console makes changes to the text contents, it must update the display
to match. "Printing" to the console is handled by calling on the gl layer to
draw text. You can make an incremental update as part of processing each
character or postpone into a single refresh after processing a string of
characters. *Your console must use double-buffering and redraw the screen
on each update.* 
